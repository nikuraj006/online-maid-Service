...
<%
  ...
  if(change!=null){
      ...
      if(pass1.equals(pass2)){
       ...
       PreparedStatement ps = conn.prepareStatement("update register set password =? where email=? or mobile=?");
       ps.setString(1,pass1);
       ps.setString(2,id);
       ps.setString(3,id);
       int t=ps.executeUpdate();
       if(t>0){session.setAttribute("PASS", pass1);
          response.sendRedirect("upasschange.jsp?cuser=Password Changed !!!");
       }else response.sendRedirect("upasschange.jsp?cuser=Password not Changed !!!");
           }catch(Exception e){
               response.sendRedirect("upasschange.jsp?cuser=SQL Exception !!!");
           }
          
      }else {
              response.sendRedirect("upasschange.jsp?cuser=Password wrong !!!");
          }
      }else{
          response.sendRedirect("upasschange.jsp?cuser=Password not Match !!!");
      }
  }
